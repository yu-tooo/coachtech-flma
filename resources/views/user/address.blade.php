<x-user-entry>
  ...
  <form method="POST" action="{{ route('user.address', ['item_id' => $item_id]) }}" class="w-full">
    ...
    <x-user.form-group name="postcode" labelName="郵便番号" value="{{ old('postcode', $profile->postcode) }}" />
    <x-input-error :messages="$errors->get('postcode')" class="-translate-y-8" />

    <x-user.form-group name="address" labelName="住所" value="{{ old('address', $profile->address) }}" />
    <x-input-error :messages="$errors->get('address')" class="-translate-y-8" />

    <x-user.form-group name="building" labelName="建物" value="{{ old('building', $profile->building) }}" />
