...
<head>
  ...
  <title>Coach Flma Manager</title>
...
<body x-data="{ open: false }">
  <header class="flex items-center justify-between px-4 h-12 bg-black">
    ...
    <div class="flex px-4 hidden space-x-4 md:block">
      @auth('admin')
      <a href="{{ route('owner.register') }}" class="inline-block text-white hover:underline">
        オーナー
      </a>
      @endauth
  ...
  <div :class="{'translate-y-24': open }" class="flex items-center flex-row-reverse absolute px-8 -top-12 bg-black h-12 w-screen duration-500">
    ...
    @auth('admin')
    <a href="{{ route('owner.register') }}" class="text-white mx-4 hover:underline">
      オーナー
    </a>
    @endauth
  </div>
